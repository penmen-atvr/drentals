...
export async function getDynamicHomeSections(): Promise<HomeSection[]> {
  try {
    ...
    return payload as HomeSection[];
  } catch (error) {
    console.error("[getDynamicHomeSections] Error:", error);

    try {
      const fallback = await buildFallbackSections();
      return fallback;
    } catch (fallbackError) {
      console.error("[getDynamicHomeSections] Fallback Error:", fallbackError);
      return [];
    }
  }
}
